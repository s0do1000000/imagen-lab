...
генераций (`users.credits` в базе), плюс **три отдельные бесплатные
попытки** — по одной на картинку, фото и видео (`free_image_used`,
`free_edit_used`, `free_video_used` в таблице `users`). Новый
пользователь может попробовать все три функции бесплатно, каждую по
разу, а дальше — покупка пакетов через Telegram Stars или USDT.
Настраивается в `lib/pricing.ts` — цены пакетов и стоимость видео в
генерациях (по умолчанию видео = 3, картинка/фото = 1).
